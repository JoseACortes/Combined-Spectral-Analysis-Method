...
Reviewer #1: The MCNP simulation model should be explained in detail, including all aspects such as the detectors used and neutron energy. The accuracy of the model should be proven with reference to the literature.

Reviewer #3: Summary: The authors M. describe in their manuscript "Simulation and Combined Spectral Analysis for In-Situ Soil Elemental Content Measurement" how Monte Carlo simulations can be used to improve the uncertainty in the retrieval of the carbon content in soil samples using a neutron generator and scintillators. The manuscript has clear scientific relevance as an applied contribution. However, its methodological gain is limited and in its current form the paper reads more as a collection of different simulation results than a conclusive study. The authors try to take into consideration a many different variables, however, that covers only a fraction of potential environmental parameters - without comparisons to measurements it is not clear how certain and relevant the current parameter space is.

The reviewer has thoroughly reviewed the manuscript and provides the following comments:
...
- Furthermore, precisely the practical parameters that would be important for the claimed application are missing: There is no statement regarding penetration depth, no quantitative information on measurement duration (or number of pulses), and no realistic experimental accuracy or uncertainty analysis.
...
Comments to the figures and tables:
...
References:
- Please format references in a consistent way (uppermost entry).