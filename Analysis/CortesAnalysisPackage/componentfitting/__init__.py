...
def convex_relative_residuals(params, x, y, alpha=1):
    residual = np.average((y - linear_combination(x, *params))**2)/np.average(y**2)
    return residual + alpha*(1-np.sum(params))**2

def find_linear_combination(x, y, convex=False, relative=False, initial_guess=None):
    # print(x.shape, y.shape)
    if initial_guess is None:
        initial_guess = [1/x.shape[0]] * x.shape[0]
    to_minimize = residuals
    # to_minimize = convex_relative_residuals
    if convex:
